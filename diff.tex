...
%DIF DEL article_old.tex   Sat Jul 19 23:00:53 2014
%DIF ADD article.tex       Mon Jul 21 16:56:35 2014
...
organ made of \DIFaddbegin \DIFadd{about }\DIFaddend 100 \DIFdelbegin \DIFdel{billions }\DIFdelend \DIFaddbegin \DIFadd{billion }\DIFaddend of neurons, each connected to 7000 other neurons on
...
. }\DIFaddend For the sake of illustration, the effect of the filter $f_1$ on the signal
...
frequency, \DIFaddbegin \DIFadd{can be seen as an indirect indicator of neuron communication }\DIFaddend while low frequencies of the signal mainly correspond to the slow
...
\DIFadd{The main idea of principal component analysis (PCA) is to reduce the
dimensionality of a dataset whose variables are interrelated
\mbox{%DIFAUXCMD
\citep{jolliffe2005principal}
}%DIFAUXCMD
. It consists in finding a new set of
...
\DIFadd{Figure~\ref{fig:pca} shows the explained variance (ratio) by compoments and gives a hint about the number of uncorrelated variables required to
explain all - or, at least, the majority of -  the variation in the original data.
}

\DIFadd{The singular value decomposition (SVD) relies on the fact that every matrix -
not only square matrices - can be decomposed into a diagonal nonnegative
matrix with the help of two unit matrices \mbox{%DIFAUXCMD
\citep{alter2000singular}
}%DIFAUXCMD
.
}

\DIFadd{The terms of the diagonal matrix can be used to determine the fraction of the
overall information hold by the components in the decomposition and therefore
give the relative significance of the components \mbox{%DIFAUXCMD
\citep{alter2000singular}
}%DIFAUXCMD
.
...
\DIFadd{Obvioulsy, Figure~\ref{fig:pca} shows that for both approaches only 800
...
\DIFadd{Therefore, the value of the parameter $M$ is immediat and should be clearly
...
\caption{\DIFaddFL{Explained variance ratio by number of principal components (left) and singular value ratio by number of principal components (right) for all networks.}}
\label{fig:pca}
\end{figure}

%DIF >  \section{Computational information}

%DIF >  To perform experiments, we used a Macbook Pro (2014) with a ram memory 16Go 1600 MHz DDR3 and a processor 2.3GHz Intel Core i7.

%DIF >  In order to compare the execution time with other methods, it takes about xx and xx hours to compute respectively GTE and GENIE3. However, GENIE3 is highly parallelizable (by taking several smaller forests or applying GENIE3 on subsets of variables) and the execution time can be significantly reduced. 

%DIF >  \begin{table}[h]
%DIF >  \caption{Execution time, scalability, memory requirements for methods described in the paper. \textcolor{red}{to do.}}
%DIF >  \label{tab:results_appendix}
%DIF >  \centering
%DIF >  \small
%DIF >  \begin{tabular}{| l|c|c|} \hline
%DIF >  Methods & Execution time  & Memory requirements \\ \hline
%DIF >  PC & & $\sim$5Go\\
%DIF >  Averaging & & \\
%DIF >  Full Method & & \\ \hline
%DIF >  \end{tabular}
%DIF >  \end{table}
\DIFaddend 
