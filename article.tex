...
$\tau$ of the true positive rate $tpr(\tau) = tp(\tau) / (tp(\tau) + fn(\tau))$
as a function of the false positive rate
$fpr(\tau) = tn(\tau) / (tn(\tau) + fp(\tau))$. In section (TODO ref),
we also compare the same methods with the area under the precision-recall
curve obtained by varying the threshold
$\tau$ of the precision $P(\tau) = tp(\tau) / (tp(\tau) + fp(\tau))$
as a function of the recall $R(\tau) = tp(\tau) / (tp(\tau) + fn(\tau))$,
