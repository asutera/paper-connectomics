...
as a set of random variables $V = \left\{V_1, \ldots, V_{|V|}\right\}$, which follows
a joint probability distribution $p_\mathcal{V}$, and by
$V^{-i,j}$ the set of all variables except $V_i$ and $V_j$.
Two random variables $V_i$ and $V_j$ are said to be conditionally independent
to a (possibly empty) set of random variables $Z$, denoted by $V_i \perp V_j | Z$,
if the joint conditional probability distribution factorizes
$p_{\mathcal{V}_i, \mathcal{V}_j|\mathcal{Z}} = p_{\mathcal{V}_i|\mathcal{Z}}
p_{\mathcal{V}_j|\mathcal{Z}}$.  The inference of the undirected network
can be formulated as inferring conditional dependences
$V_i \not\perp V_j | V^{-i,j}$ between all pair of fluorescence signal
$i$ and $j$, in other words the inferred network will be
$\hat{A}: \hat{a}_{i,j} = I(V_i \not\perp V_j | V^{-i,j}), i, j \in V$.

Since all neurons are assumed to be connected to at least one neuron,
we thus have by definition $V_i \not\perp V_j$. However,  two random
variables $A$ and $B$ might be dependent taken alone ($A \not\perp B$), but not
once we observe a third variables $C$ ($A \perp B | C$).
The conditioning over all other random variables $V^{-i,j}$ is thus necessary
to avoid predicting an edge when two neurons are not directly connected.

If the random variables $V$ follows a joint Gaussian distribution
$p_\mathcal{V} \sim \mathcal{N}(\mu; \Sigma)$ of mean vector $\mu$ and
covariance matrix $\Sigma$, we have the conditional independence
$V_i \perp V_j | V^{-i,j}$ if the partial correlation coefficient
\[
\rho_{V_i, V_j | V^{-i,j}}
= \frac{\Sigma^{-1}_{ij}}{\sqrt{\Sigma^{-1}_{ii} \Sigma^{-1}_{jj}}}.
\]
is equal zero. The partial correlation gives a connectivity score
$\hat{A}: \hat{a}_{i,j} = \rho_{V_i, V_j | V^{-i,j}}, i, j \in V$ between all
pair of neuron $i$ and $j$. One could retrieve an estimated graph by thresholding
the matrix $\hat{A}$.
By contrast, we also have $V_i \perp V_j$ if the Pearson correlation
coefficient
\[
\rho_{V_i,V_j} = \frac{\Sigma_{ij}}{\sqrt{\Sigma_{ii}
\Sigma_{jj}}}
\]
is equal to zero.

% Speak about why averaging
The quality of the signal filtering and network inference depends
...
In supervised learning term, the network inference task can be viewed as a
...
($(i, i) \not E, \forall i \in V$) and some inference methods could spuriously
leave maximal scores to the diagonal of $\hat{A}$, e.g. Pearson correlation,
we set those elements to the minimum of $\hat{A}$ prior evaluating those
algorithms.
