...
pair $(i,j)$ an empirical estimation of the  term:
\begin{align}
s_{i,j} = \sum_x \sum_{t=1}^{T - 1} \mathbb{1}((x_j^{t+1} - x_i^t) \in \left[\phi_1, \phi_2\right])
\end{align}
...
$0.2$ and $0.5$, respectively. Their role is
...
$p_{i,j}$ so as to take into account directionality. Naturally, îf
...
This suggests the the new association matrix $r$:
\begin{align}
