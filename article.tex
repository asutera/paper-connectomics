...
independences. Each neuron signal $i$ is a random continuous variable $X_i$,
...
and $X_j$ are independent $X_i \perp X_j$ if they have zero correlation
coefficient and are conditionally independent to all others neurons
$X_i \perp X_j | X_1,\ldots,X_p \backslash \left\{X_i, X_j\right\}$ if they
have zero partial correlation coefficient. Even if \textit{independence
does no imply causation}, it gives insight on the underlying connectivity
network.

% Introduce correlation => Estimator of I(X; Y) => Issue indirect effect
The Pearson correlation coefficient $\rho_{ij}$ is an association measure
and defined by
\[
\rho_{ij} = \frac{\Sigma_{ij}}{\sqrt{\Sigma_i \Sigma_j}}.
\]
