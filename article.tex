...
analysis (PCA) as to respectively determine a set of independent linear combinations of the
...
The distribution of compoment eigen values obtained from PCA and SVD decompositions can be studied by sorting them in descending order of magnitude as illustrated on Figure~\ref{fig:pca}. It can be seen that some component eigen values are zero implying that the behavior of the network could be explained by a subset of neurons because of the 
redundancy and relations between the neurons. For all datasets, the eigen value distribution is exactly the same. 

In the context of the challenge, we observe that only $800$ components seem to be necessary and we exploit this when computing partial correlation statistics. Therefore, the value of the parameter $M$ is immediat and should be clearly set to $800$, or equivalently to $0.8p$.

Note that if the true network is not available, similar decomposition analysis could be carried on the infered network or on the data directly.\\
