...
The connectomes, the neural network, is represented by a (un)directed graph
$G = (V, E)$ comprising
a set of nodes $V$ and a set of edges $E \subseteq \left\{(i, j) \in V \times
V\right\}$, (un)ordered pairs from $V$.
Each node $i \in V$ represents a neuron, and each edge $(i, j) \in E$
represents a direct link and interaction between neuron $i$ and $j$.
In the directed graph, edges also express causality: the
state of a neuron $i$ might be due to the state of one or several of its
parents $\left\{j : (j, i) \in E, j \in V\right\}$.
...
empirical analysis and comparison with other network inference methods,
