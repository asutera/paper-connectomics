...
filter pattern. Formally, the low-pass filter 

\begin{align}
y_{stacking} = \sum_{k=1}^{k=4}  w_k \sum_{f_3 \in T} f(X, k, f_3)
\end{align}
where $f(X, k, f_3)$ applies the filtering and inference methodologies (as described 
in sections \ref{sec:filter} and \ref{sec:inference}) with a  given value $f_3$ for the
hard-threshold among a set of thresholds $T$ and the $k^{th}$ low-pass filter pattern.
...
\cite{de2004discovery}.}\cite{de2004discovery}.
