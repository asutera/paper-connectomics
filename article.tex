...
\subsection{Partial correlation compare favorably with other state-of-the art methods}
...
pre-processing. \textcolor{red}{Those results generalized over the other
datasets see Appendix REF.}
...
If one would infer perfectly the undirected network, it would already yield
...
small AUROC gain. The area under the ROC curve is not a consistent metric
...
For these reasons, we strongly advice to avoid the AUROC
for the assessment network inference using a more appropriate
metric such as the area under the precision-recall curve (AUPRC). Those
