...
The full signal processing of our approach is defined by
\begin{align}
w(h(g(f_k(x_i^t))))
\end{align}
...
We compare our approach to the Pearson correlation, the generalized transfer
entropy (GTE) and GENIE3.

GTE, the baseline of the challenge, measures for all pairs of
...
GENIE3 (\cite{huynhthu2010inferring}) translates the inference of the
network of $p$ neurons into $p$ significant variable tasks. For each task,
the fluorescence level of a neuron is predicted from the fluorescence
of all other neurons using a tree based ensemble such as
random forest (\cite{breiman2001random}). The importances
(\cite{louppe2013understanding}) given by the tree ensemble to predict
the time series of a neuron given the others are used to infer
a connectivity score matrix. The following parameters are used
