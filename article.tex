...
the challenge dataset and this tuning was probably decisive to get the best
performance, we believe that the tuned method clearly overfits the
...
In Section~\ref{sec:filter}, we introduce four filtering functions ($f$, $g$,
...
following function: $w^*\circ p \circ h \circ g \circ f_i$ (with $i=1$,2,3, or 4)
...
varying the low-pass filter in $\{f_1, f_2, f_3, f_4\}$. Unlike in the
simplified method where each filter $f_1$ and $f_2$ were given equal weight in
the averaging, in the tuned method, we weighted each of the four filters
according to its individual performance on the \textit{normal-1}
dataset. \textcolor{red}{PG: je ne suis pas s\^ur de comprendre comment ces
  poids sont d\'etermin\'es}
...
time step, ie., when the delay between the two activations is below the time
...
$s_{i,j}$ is lower than $s_{j,i}$ ($i\rightarrow j$). To address these issues,
we computed a differential pairwise coefficient $z_{ij}$ defined as follows:
\begin{align}
z_{ij} = \sum_{t=1}^{T - 1}
...
\textcolor{red}{PG: je ne comprends pas ce qui est fait exactement. Mettre les
  maths? Valeurs de $\phi_1$ et $\phi_2$ ?} At the light of the poor results of this heuristic taken on its own
(on Table \ref{tab:directivity} in Section~\ref{sapp:results}) as
\textit{directivity}, we use it to make asymmetric the degree of association
...
We report in this apprendix the performance of the different methods compared
