...
Intuitively, connectivity information between two
...
case of an undirected relationship, it might change. Therefore, the
...
and we thus choose to consider the discrete derivative of the signal, i.e. a
high-pass filtered version, $g_2$:  \begin{align} g_2(X_t^i) = X^i_t -
X^i_{t-1} \end{align} where ... .
...
informations. We thus apply the filter
$g_3$: 
\begin{align} g_3^k(X^i_t) =
...
\textcolor{red}{miss the case where activity < 0->1} where $r(x)$ is a 4-piecewise continuous functions whose parameters were tuned
...
\begin{align}
G^{q,k}(X_t^i) = h_2 \bullet h_1 \bullet g^q_1 \bullet g_2 \bullet g_3^k(X_t^i).
\end{align}
...
the eigen vectors which have the largest eigen value \textcolor{red}{fig?}.
...
Prior inferring the network, the fluorescent calcium
...
filter are applied for each possible time step $t$ % TODO fix first equation
