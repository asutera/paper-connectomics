...
\end{align}
that has for effect to magnify the importance of spikes that occur in case of low global activity. Note that the particular case where $\sum_{j} x^{t}_j$ equals $0$ is solved by setting $w(x^{t}_i) = 1$.
