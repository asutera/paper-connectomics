...
\end{align}
where $T$ is the number of time steps, $\phi_1$ and $\phi_2$ are parameters. The coefficient $z_{i,j}$ estimates the confidence in an edge $i\rightarrow j$. Note that if $z_{i,j}$ is positive, then $z_{j,i}$ will be negative.
