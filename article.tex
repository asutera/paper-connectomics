...
% We then define our preprocessing function $G$ as
% \begin{align}
% G(X_t^i) = g^q_1(g_3(g_2(X_t^i))).
% \end{align}
...
\begin{align}
h_1(X_t^i) = (X_t^i + 1)^{\left( 1 + \dfrac{1}{a_t} \right)} &\text{ with } a_t = \sum_{i=1}^p \left ( X_t^i + \dfrac{1}{2} X_{t-1}^i \right ) \\
h_2(X_t^i) = (X_t^i)^{r(a_t)} &\text{ with } r_t = \frac{a_t}{\max_{l \in T}{a_l}} 
\end{align}

where $r(x)$ is a 4-piecewise continuous functions whose parameters were tuned
for each low-pass filter.

% Hence, we define our preprocessing function $H$ as 
% \begin{align}
% H(X_t^i) = h_2 \bullet h_1 \bullet G(X).
% \end{align}
...
over all timesteps:
\[
z_{ij} = \sum_{t=1}^n \mathbb{1}(X_{t+1,j} \ge X_{t, i}) \cdot \mathbb{1}(f_1\le X_t^i \le f_2) -  \mathbb{1}(X_{t+1,i} \ge X_{t, j}) \cdot \mathbb{1}(f_1 \le X_t^j \le f_2)
\] 
