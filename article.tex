...
The connectomes, defined by the graph $G = (V, E)$,  can be represented by an
adjacency matrix $A: a_{i,j} = \mathbb{1}((i, j) \in E)$. The matrix can be
decomposed through a singular value decomposition (SVD) or principal components
analysis (PCA) as to determine a set of independent linear combinations of the
variable \citep{alter2000singular} or respectively a reduced  set of linear
combination combine which maximize the explained variance of the data
\citep{jolliffe2005principal}. Otherwise said, both SVD and PCA aims at finding
a reduced set of neurons whose activity can explain the rest of the network.
Note that those two methods while having slightly different objective are
related.

From all the available datasets, one can study the distribution of eigen value
obtained from PCA and SVD decomposition by sorting them in descending order of
magnitude as illustrated on Figure \ref{fig:pca}. For both methods, the  200
lowest eigen values are zero. Thus, the behavior of the network could be
explained by 800 linear combination of the variables which could be explained
by the redundancy and relation between the neurons.

In the context of the challenge, we exploit this information to keep only  $M =
0.8p$ components when computing the partial correlation statistics. Without
those comparative network, one could perform a similar decomposition analysis
on the data.
...
\caption{Explained variance ratio by index of principal components (left) and singular value ratio by index of principal components (right) for all networks sorted by descending order of magnitude.}
\label{fig:pca}
\end{figure}
