...
filtering and infering processes. One approach, further referred
...
and about 15000 edges each (i.e., a network density of about
...
summarized using the area under the curve and respectively  denoted AUROC
...
severly affects the performance of this method with respect to the use of fully
grown trees. We nevertheless provide these results for comparison purpose. PC
and GENIE3 were both applied on the same pre-filtered time series as partial
correlation ($h\circ g \circ f_1$). For GENIE3, we built 10000 trees per neuron
and we used default settings for all other parameters (except for the maximal
tree depth). For GTE, we reproduced the exact same setting (conditioning level
and pre-processing) that was used for the challenge.
...
higher than 0.5 (see Figure \ref{fig:curves}(b)), suggesting that it ranks high
mainly pairs of neurons that interact in both directions (there are about XX\%
of such pairs in the \textit{normal-2} network). %% It is interesting also to note
%% that, on \textit{normal-2}, a method that perfectly predicts the undirected
%% network (i.e. that gives a score of 1 to each pair $(i,j)$ such that $(i,j)\in
%% V$ or $(j,i)\in V$, 0 otherwise) can reach an AUROC of $0.995$, which does not
%% leave much room for improvement by predicting edge directionality. The AUPRC on
%% the other hand would be 0.789 and thus be more sensitive to the prediction of
%% edge directions.
...
\hline
\hline
Partial correlation ($f_1$ + $f_2$) & 0.937 & 0.935 & 0.935 & 0.931 & 0.391 &  0.390 &  0.385 & 0.375  \\
Partial correlation ($f_1$ + $f_2$ + $f_3$ + $f_4$) & \textbf{0.938} & \textbf{0.936} & \textbf{0.936} & \textbf{0.932} & 0.391 & 0.389 & 0.385 & 0.374\\
\hline
\end{tabular}
...
\begin{table}[h]\label{tab:comparison_extended}
\centering
\small
