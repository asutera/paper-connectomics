...
only connected if they correspond to two dependent variables. We consider that two neurons are
...
\end{equation}
where $\Sigma^{-1}$ is also known as the precision matrix or concentration. 
...
X^{-j,i}}$). Therefore, our approach cannot identify the direction of the interaction. We will see why this only affects slightly its performances, with respect to the metric used in the Connectomics challenge, in Section~\ref{sec:results}.

We detail now our procedure for estimating partial correlation statistics ($\rho_{X_i, X_j | X^{-i,j}}$) from
the data. Besides the straightforward computation, a natural procedure for estimating this precision matrix is first computing an empirical estimation of the covariance matrix and then taking the inverse, applying equation \ref{eq:inverse}. 
Yet, an approximation can be obtained by only considering the first $M$principal components. Notice that we could have expected that a approximation of the inverse would have degraded the performance. However, the opposite was observed. We conjecture that by only taking into account the $M$ principal components filter out noise. In our experiments, $M$ is set to $0.8p$ (see discussion in Appendix~\ref{app:REF} \textcolor{red}{REF}).
