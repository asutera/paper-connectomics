...
$\tau$ and the design of the low-pass filter (mentionned in section
\ref{sec:filter} and appendix \ref{app:optimized}).
...
As it can also be seen on Figure \ref{fig:curves}, the GENIE3 performance increases with deeper trees. The reason is twofold. First, the deeper the tree, the larger is the conditioning. As it was observed in the comparison between correlation and partial correlation, it screens indirect links in the infered network. Second, the number of trees when trees are small need to be much higher in order to correctly estimate all pairs. However, as suggested by \cite{Gilles2014understanding}, limiting the maximal depth of trees may help to reduce the misestimation bias in variable importance comuting, i.e. infering the strengh of a link. 
...
If the undirected network is perfectly, it would already yield
...
with respect to the problem statement. We advice to avoid the AUROC
