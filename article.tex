...
weight in the averaging of the simplified method. In contrast, filters $f_3$
...
this section, we present a heuristic to retrieve directionality. %% Note however that
%% given the discussion at the end of Section~\ref{sec:results}, we cannot expect
%% a very large improvement in AUROC by predicting edge orientation.
...
\end{align}
where $\phi_1$ and $\phi_2$ are parameters of the heuristic (with $\phi_1 = 0.2$ and $\phi_2 = 0.5$ in our experiments).
...
\begin{align}
z_{i,j} = s_{i,j} - s_{j,i}.
\end{align}
The coefficient $z_{i,j}$ estimates the confidence in an edge $i\rightarrow j$. Note that if $z_{i,j}$ is positive, then $z_{j,i}$ will be negative.

This heuristic does not give good results on its own (see Table \ref{tab:directivity} in Section~\ref{sapp:results}). However, it can be used to asymmetrize the degree of association (symmetric) matrix given by partial correlation statistics. Concretely, a new matrix is computed as the linear combination of the partial correlation matrix and the heuristic matrix.
Mathematically, we have
\begin{align}
q_{i,j} = weight_p * p_{i,j} + weight_z * z_{i,j}
\end{align}
where $weight_p$ and $weight_z$ are parameters of the method (with $weight_p = 0.997$ and $weight_z = 0.003$ in our experiments).

Considering the small weight associated to this heuristic, the effect is subtle. It slightly increases the confidence in one edge direction and decreases the confidence in the opposite edge direction while not affecting strongly the undirected topology of the network. This process slightly improves the performance of our method (see Table \ref{tab:directivity} in Section~\ref{sapp:results}).
...
GENIE3 & 0.781 &  0.922 & 0.875 & 0.899 & 0.886 &  0.890 \\ \hline \hline
