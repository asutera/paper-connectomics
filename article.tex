...
As the Figure~\ref{fig:original_curve} shows, the raw
...
Neurons communicate through short spikes, characterized by a high
frequency, while low frequencies of the signal correspond to the slow
decay of fluorescence. The second step of our pipeline consists in
a high-pass filter that transforms the time series into its backward
difference
\begin{align}
g(x^{t}_{i}) &= x^{t}_i - x^{t-1}_i. \label{eq:high-pass-filter},
\end{align}
resulting in the plot of Figure \ref{fig:hp_curve}:

To further filter out small signal variations, mostly due to noise, the third
step consists in a  hard-threshold filter
\begin{align}
...
% check dependency map => minimal d-map with distribution?
\textcolor{red}{This part is better, however there is still something
wrong with the theory.}
To a joint probability distribution $P_X$ corresponds a minimal dependency
graph. Each node of this graph is a random variable. Two random variables are
only connected if they are dependent. We consider that two neurons are
connected if  and only if,  there exists  in this dependency  graph an edge
that connects  directly the  two random  variables to  which they correspond.
This is equivalent to writing that two neurons $i$ and $j$ are  not  directly
...
Our approach implicitly makes the additional assumption  that $P_X$ is  a joint
Gaussian distribution since it  outputs as  degree of association between two
neurons $i$ and $j$ an estimate of $\rho_{X_i, X_j | X^{-i,j}}$. Note also that
the partial correlation statistic is symmetric  (i.e., $\rho_{X_i, X_j |
X^{-i,j}} = \rho_{X_j, X_i | X^{-j,i}}$) and therefore does not account for the
direction of the interaction.


We detail now our procedure for estimating $\rho_{X_i, X_j | X^{-i,j}}$ from
the data. \textcolor{red}{DE finds that this sections is too complicated}
The partial correlation statistics is computed through the inverse
$\Sigma^{-1}$ of the covariance matrix $\Sigma$, also known as the
concentration or precision matrix. Computationally, they can be estimated
efficiently in $O(M^3)$ (instead of $O(p^3)$) from the $M$ first principal
components extracted through PCA~\citep{bishop2006pattern} on the processed
data. In our experiments, $M$ is set to $0.8 p$, thereby allowing us to further
filter the the data by removing the contribution of principal components likely
due to noise.
...
and $X_j$ are independent if their Pearson correlation statistic is zero, that
is if
\begin{equation}
