...
the differences about the signal processing methods applied on fluorescent
calcium time series; Section~\ref{sapp:averaging} then provides a detailed
explanation of the averaging approach; Section~\ref{sapp:connectome} presents
a heuristic trying to retrieve directivity from fluorescent calcium time
series while Section~\ref{sapp:results} finally present results validating
some choices that have been made.
...
filtering composed function. On the one hand, each part of the composed functions is optimized to maximize the ROC AUC (e.g., $w^*$). On the other hand, we introduce new functions (e.g., $f_3$ and $f_4$) leading to alternative composed functions. They might lead to worse ROC AUC when taken individually but they improve the overall ROC AUC when taking as a part of the averaging approach.
...
The AUROC performance of the conjunction of the composed function $w^* \circ p
\circ g \circ h \circ f_1$ (respectively $f_2$, $f_3$ and $f_4$) and the
partial correlation statistics was quite sensitive to the choice of $\tau$, the
hard threshold value and the design of the low pass filter. Similarly to the
simplified method, we average the partial correlation statistics obtained by
varying $\tau$ linearly from $0.100$ to $0.209$ and the low-pass filter.
...
additional gain margin should be extremely limited.
...
At the light of the poor results of this heuristic taken on its own
(on Table \ref{tab:directivity} in Section~\ref{sapp:results}) as \textit{directivity}, we use it to make
...
As reported on Table~\ref{tab:f3f4}, averaging over all low-pass filters leads to better ROC AUC than averaging over only two low-pass filters, i.e., $f_1$ and $f_2$.
...
\paragraph{On the contribution of the directivity heuristic.}
\begin{table}[ht]\label{tab:comparison}
%\caption{Evaluation of different methods on the four \textit{normal} datasets\label{tab:comparison}}
\centering
\small
...
Average partial correlation and its tuned variant, i.e.,``Full method'', clearly outperform other network inference methods on all datasets (with 1000 neurons) provided by the organizers. It confirm that the presented methodology in this work would constitute a very solid baseline. 
...
% \hline
% & \multicolumn{6}{c|}{AUROC} & \multicolumn{6}{c|}{AUPRC} \\
% \textit{Method} $\backslash$ \textit{normal-} & \textit{lowcon} & \textit{highcon} & \textit{lowcc} & \textit{highcc} & \textit{3-highrate} & \textit{4-lownoise} & \textit{lowcon} & \textit{highcon} & \textit{lowcc} & \textit{highcc} & \textit{3-highrate} & \textit{4-lownoise}\\
% \hline
% \hline
% % No  filtering       					& 0.777 & 0.767 & 0.772 & 0.774 & 0.070 & 0.064 & 0.068 & 0.072\\
% % $ h \circ g \circ f_1$                  & 0.923 & 0.925 & 0.923 & 0.922 & 0.311 & 0.315 & 0.313 & 0.304\\
...
% Partial correlation (avg. 2) & 0.937 & 0.935 & 0.935 & 0.931 & 0.391 &  0.390 &  0.385 & 0.375  \\
% Partial correlation (avg. 4) & 0.938 & 0.936 & 0.936 & 0.932& 0.391 & 0.389 & 0.385 & 0.374\\
...
% Our approach (opt. \& avg. 4)    & 0.943 & 0.942 & 0.942 & 0.939 & 0.403 & 0.404 & 0.398 & 0.388 \\
