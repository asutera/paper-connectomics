...
filtering functions. Except for the last two rows that uses PCA, the
...
\textit{Method} $\backslash$ \textit{normal-} & 1 & 2 & 3 & 4 & 1 & 2 & 3 & 4 \\
\hline
\hline
...
\hline
\end{tabular}
\caption{Top: ROC (left) and PR (right) AUC on \text{normal-1,2,3,4} with partial correlation and different filtering functions.
Bottom: ROC (left) and PR (right) AUC on \text{normal-1,2,3,4} with different methods.}
\end{table}
...
Forests model to a maximum depth of 3. This constraint potentially severly
...
\caption{ROC (left) and PR (right) curves on \text{normal-2} with different methods. Areas under the curves are reported in the legend.}
