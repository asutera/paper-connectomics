...
Let us assume that the fluorescence concentrations of all $p$ neurons modeled
as a set of random variables $X = \{X_1, \dots, X_p\}$ are drawn from a time
invariant joint probability distribution $P_X$.
In the probability framework, two random variables $X_i$ and $X_j$ are
...
is null~\citep{baba2004partial}, i.e., if
\begin{equation}
...
X^{-i,j}} \neq 0$, where $\rho_{X_i, X_j | X^{-i,j}} \in [-1;1]$ can now be
interpreted as the degree of association between the two variables (i.e., the larger the
value, the stronger the interaction). In practice due to the finite size of
time series, noise and modeling assumptions, the partial correlation
$\rho_{X_i, X_j | X^{-i,j}}$ will never be zero. If instead of a degree
of association, one would want to infer the underlying graph a statistical
test would be required to assert conditional independences. Note also that
...
experiments, $M$ is set to $0.8 p$, thereby allowing us to further filter
