...
organ made of about 100 billions of neurons, each connected to 7000 other neurons on
...
\end{align}
These filters are standard in the signal processing field \citep{kaiser1977data, oppenheim1983signals}. The idea of the second one is to not consider future values and to introduce decreasing weights. For the sake of illustration, the effect of the filter $f_1$ on the signal
is shown in Figure \ref{fig:lp_curve}.

Furthermore, short spikes, characterized by a high
frequency, can be seen as an indirect indicator of neuron communications while low frequencies of the signal mainly correspond to the slow
...
\citep{jolliffe2005principal}. It consists in finding a new ordered set of
uncorrelated variables, called the principal components, where the most
principal components, i.e. the first variables, hold most of the data variation, also denoted as
variance, scattered in all the original variables.

Figure~\ref{fig:pca} shows the explained variance by compoments and gives a hint about the number of uncorrelated variables required to
explain all the variation in the original data.
...
Therefore, the value of the parameter $M$ is immediat and should be obviously
...
\caption{Explained variance ratio by component (left) and singular value ratio by component (right) for all networks.}
\label{fig:pca}
\end{figure}
...
\begin{table}[h]
\caption{Execution time, scalability, memory requirements for methods described in the paper. \textcolor{red}{to do.}}
\label{tab:results_appendix}
\centering
\small
\begin{tabular}{| l|c|c|c|} \hline
Methods & Execution time & Scalability & Memory requirements \\ \hline
Partial correlation & & & \\ 
GTE  & & & \\
GENIE3 & & & \\ \hline
\end{tabular}
