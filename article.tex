...
% introduce the notion of dependance/independance
% TODO add a more formal definition of independance.
From a probabilistic point of view, the inference of the undirected network
can be formulated as inferring the conditional independence between neurons
$i$ and $j$ conditionally to all other neurons
$i \perp j | 1,\ldots,p \backslash \left\{i, j\right\}$. Note that
the unconditional independence $i \perp j$ between  $i$ and $j$ doesn't allow to
infer the graph due to indirect effect. For instance if neurons $A, B, C$ have
the connectivity graph $A \rightarrow B \rightarrow C$, then we have the
dependence $A \not\perp C$ and the conditional independence $A \perp C | B$.



% Introduce correlation => Estimator of I(X; Y)
%                       => Issue indirect effect
% Introduce partial correlation => Estimator of I(X; Y | Z)
%                               => Symmetric measure
% Introduce a custom solution to make the Y matrix assymetric
% Introduce Filtering
