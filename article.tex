...
symmetrical median filter, i.e., $X_t^i \leftarrow X_{t-1}^i + X_{t}^i + X_{t+1}^i$, 
and an asymmetrical weighted median filter, i.e., 
$X_t^i \leftarrow 0.4 X_{t-3}^i + 0.6 X_{t-2}^i + 0.8 X_{t-1}^i + X_{t}^i$.}\\
...
an asymmetrical discrete derivative : $X_{t,d}^{i} \leftarrow X_{t}^i - X_{t-1}^i$.}\\
...
$X_{t,d}^i = X_{t,d}^i \mathbb{1}(X_{t,d}^i \geq \tau)$ 
...
\textcolor{red}{Then we apply on the entire signal a non-linear filter base on the 
...
When too much neurons are firing together, i.e. a network burst, it is almost 
...
we thus have by definition $X_i \not\perp X_j$ \textcolor{red}{AS: Je ne suis pas sûr de comprendre.}. However,  two random
