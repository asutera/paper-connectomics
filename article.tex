...
In the context of the Connectomics challenge, we propose a simple algorithm made of four stages to achieve a network inference of high quality.
...
\end{align}
where $f(X, k, f_3)$ gives the predicted adjacency matrix, i.e. applies the filtering 
and inference methodologies (as described in sections \ref{sec:filter} and \ref{sec:inference}), 
with a  given value $f_3$ for the hard-threshold among a set of thresholds $T_k$ 
of size $|T_k|$ and the $k^{th}$ low-pass filter pattern associated to a weight $w_k$ 
whose values are $w_1 = 0.01 , w_2 =1, w_3 = 0.7, w_4 = 0.9$.
...
In this section, we will present some experiments on challenge datasets to show 
the good performance of our method and compare it to other inference methods. 
Since it was the metric used during the challenge, we will compare inference methods
regarding the area under the ROC curve. However, we will discuss about this in section
\ref{sec:metric}.


\paragraph{Datasets \\}
To assess the quality of our method, we use four network, denoted as \textit{normal-\{1,2,3,4\}}, similar to networks, i.e., the same number of neurons, used to rank challenge participants. Note that our parameter tuning was mainly on \textit{normal-1} dataset and partly on \textit{normal-4} dataset, therefore it may justify why these networks are better retrieved despite the fact that some might be simpler to infer.  
